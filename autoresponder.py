...
# Отключаем цензуру Google (чтобы ИИ не блокировал маты и сленг)
safety_settings = [
...
# Явно перехватываем бизнес-сообщения
@dp.business_message()
async def handle_business_message(message: types.Message):
    ...
    if ai_reply:
        try:
            # Отправка сообщения от имени владельца бизнес-аккаунта
...
async def main():
    ...
    # Очищаем вебхук на всякий случай перед стартом
    await bot.delete_webhook(drop_pending_updates=True)
    
    # ВАЖНО: Явно указываем, какие апдейты мы хотим получать.
    # Это необходимо для бизнес-сообщений в новых версиях aiogram.
