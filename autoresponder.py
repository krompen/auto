...
async def process_show_prompt(callback: types.CallbackQuery):
    if callback.from_user.id != ADMIN_ID: return
    settings = load_settings()
    # Теперь отправляем промпт обычным сообщением, чтобы избежать ошибки лимита символов
...
async def handle_business_message(message: types.Message):
    ...
    if ai_reply:
        try:
            # Явное указание ID бизнес-подключения, чтобы бот отвечал от вашего лица
